...
import sys
import os
# Ensure the folder that contains the top-level `distiller` package is on sys.path.
# Previously we were appending the `apputils` subfolder which made Python import a
# partial package and prevented sibling modules (e.g. distiller.utils) from being found.
script_dir = os.path.dirname(__file__)
# The `distiller` package root is the `distiller` folder inside the project root.
distiller_root = os.path.abspath(os.path.join(script_dir, "distiller"))
if distiller_root not in sys.path:
    sys.path.insert(0, distiller_root)

import distiller
import shap
import torchnet.meter as tnt
from distiller import apputils
from distiller.apputils import checkpoint, data_loaders, image_classifier, performance_tracker
